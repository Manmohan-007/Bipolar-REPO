...
class ProductsLeft extends Component {
  state = {
    ProductData: JSON.parse(localStorage.getItem("ApiDataBase")).productsPage
      .products,
    checkArr: [],
  };
    ...
    const MappedTableData = this.state.ProductData.map((item, pos) => {
      return (
        <tr className={classes.MainRow} key={pos}>
          ...
          <td className={classes.dltIcon}>
            <span
              onClick={(e) => this.handleDlt(pos)}
        ...
        <div className={classes.ProductTableWrapper}>
          <div className={classes.ProductLandingTableWrap}>
            ...
            <table className={classes.TableData}>
              <tbody>{MappedTableData}</tbody>
            </table>
          </div>
        </div>
        <Link to="/products/add">
          <button className={classes.TableBtn}>Add New Product</button>
        </Link>
        <button
          className={classes.TableDeleteBtn}
          onClick={this.handleMultipleDlt}
        >
